    ...
    [(); P::WIDTH]:,
{
    ...
    // let get_at_index = |comm: &'a PolynomialBatch<F, C, D>, i| comm.get_lde_values(i, step);
    let get_trace_at_indices = |i_start| -> [P; S::COLUMNS] {
        trace_commitment
    ...
    let quotient_values = (0..size)
        ...
        .map(|i_start| {
            ...
            let lagrange_basis_last = *P::from_slice(&lagrange_last.values[i_range.clone()]);
            ...
            let vars = StarkEvaluationVars::<F, P, { S::COLUMNS }, { S::PUBLIC_INPUTS }> {
                local_values: &get_trace_at_indices(i_start),
                next_values: &get_trace_at_indices(i_next_start),
